...
DID_HIGH = 0x00
DID_LOW = 0x44
WRITE_DATA = [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]
...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog_刷写流程-01-002_{timestamp}.txt")
...
def main():
    ...
    # ---------- nana安全访问：仅尝试 27 01，失败也继续 ----------
    logger.log("\n[安全访问] 尝试请求种子 (27 01)")
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, [SID_27, SUB_REQUEST_SEED],
                       expected_positive_sid=SID_27+0x40, allow_nrc=True)   # 允许否定响应

    if resp is None:
        logger.log("请求种子无响应，继续执行 34 测试", is_error=False)
    elif resp.responseType == 0:  # 否定响应
        logger.log(f"请求种子否定响应 (NRC=0x{resp.NRC:02X})，继续执行 34 测试", is_error=False)
    else:  # 正响应
        seed = resp.data[1:] if len(resp.data) > 1 else b''
        logger.log(f"种子: {seed.hex().upper()}")
        # 发送错误密钥（子功能 02）
        logger.log("\n[安全访问] 发送错误密钥 (27 02 + 全0xFF)")
        wrong_key = bytes([0xFF] * len(seed))
        req_key = [SID_27, SUB_SEND_KEY] + list(wrong_key)
        resp2 = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, req_key,
                            expected_positive_sid=None, allow_nrc=True)
        if resp2 is None:
            logger.log("发送密钥无响应，继续执行 34 测试", is_error=False)
        elif resp2.responseType == 0 and resp2.sid == SID_27:
            logger.log(f"安全认证失败触发, NRC=0x{resp2.NRC:02X}")
        else:
            logger.log("发送密钥未触发否定响应，继续执行 34 测试", is_error=False)

    # ---------- 测试 34 服务 ----------
    logger.log("\n[测试] 发送 34 00 44 写数据请求")
    req_34 = [SID_34, DID_HIGH, DID_LOW] + WRITE_DATA
    logger.log(f"请求: {bytes(req_34).hex().upper()}")
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, req_34,
                       expected_positive_sid=None, allow_nrc=True)

    test_passed = False
    if resp is not None and resp.responseType == 0 and resp.sid == SID_34:
        logger.log(f"否定响应正确: 7F 34 {resp.NRC:02X}")
        test_passed = True
    else:
        logger.log("未收到期望的否定响应", is_error=True)
