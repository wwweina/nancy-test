...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-双分区流程-01-012_{timestamp}.txt")
...
# ==================== 主流程 ====================
def main():
        # 初始化 test_passed 为 True，遇到错误再设为 False
    test_passed = True
    ...
    if resp is None or resp.responseType == 0:
        logger.log("进入默认会话失败", is_error=True)
        test_passed = False
    else:
        logger.log("进入默认会话成功")
    ...
    if resp is None or resp.responseType == 0:
        logger.log("安全认证失败", is_error=True)
        test_passed = False
    else:
        logger.log("安全认证成功")
