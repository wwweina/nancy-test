...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-刷写流程-01-021_{timestamp}.txt")
...
def main():
    ...
    if resp is None:
        logger.log("进入扩展会话：无响应", is_error=True)
        test_passed = False
    elif resp.responseType == 0:
        logger.log(f"扩展会话收到否定响应: NRC=0x{resp.NRC:02X} -> 失败", is_error=True)
        test_passed = False
    else:
        logger.log("进入扩展会话成功")
    ...
    if resp is None:
        logger.log("部件重启：无响应", is_error=True)
        test_passed = False
    elif resp.responseType == 0:
        logger.log(f"部件重启收到否定响应: NRC=0x{resp.NRC:02X} -> 失败", is_error=True)
        test_passed = False
    else:
        logger.log("部件重启成功")
