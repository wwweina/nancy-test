...
def main():
    ...
    logger.log("\n[测试] -------------开始测试1003---------")
    req_10 = [SID_10, SUB_EXTAND_SESSION, 0X00]
    ...
    if resp is None:
        ...
        logger.log(f"收到否定响应: NRC=0x{resp.NRC:02X} -> 符合预期", is_error=True)
        test_passed = True
    else:
        logger.log("收到肯定响应 ->失败 ", is_error=True)
        test_passed = False


    logger.log("\n----------物理寻址下预编程检查----------")
    request = [SID_31, SUB_DEFAULT_SESSION, 0x2, 0x3]
