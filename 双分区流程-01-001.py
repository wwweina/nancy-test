...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-双分区流程-01-001_{timestamp}.txt")
...
def main():
    ...
    test_passed = True #增加test_pass初始化
    ...
    if resp is None or resp.responseType == 0:
        logger.log("进入扩展会话失败", is_error=True)
        test_passed = False
    else:
        logger.log("进入扩展会话成功")

    logger.log("\n----------读取当前运行分区----------")
    request = [SID_22, 0xF0, 0xF0]
    logger.log(f"请求: {bytes(request).hex().upper()}")
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, request,
                       expected_positive_sid=SID_22 + 0x40, allow_nrc=True)
    if resp is None or resp.responseType == 0:
        logger.log("读取运行分区失败", is_error=True)
        test_passed = False
    else:
        logger.log("读取运行分区成功")
    ...
    logger.log("\n----------阻止切分区----------")
    request = [SID_31, 0x01, 0xDD, 0x0F]
    logger.log(f"请求: {bytes(request).hex().upper()}")
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, request,
                       expected_positive_sid=SID_31 + 0x40, allow_nrc=True)
    if resp is None:
        logger.log("阻止切分区：无响应", is_error=True)
        test_passed = False
    elif resp.responseType == 0:
        logger.log(f"阻止切分区收到否定响应: NRC=0x{resp.NRC:02X} -> 失败", is_error=True)
        test_passed = False
    else:
        logger.log("阻止切分区成功")
    ...
    logger.log("\n----------硬重置后再次读取运行分区----------")
    request = [SID_11, 0x01]  # 硬重置
    logger.log(f"请求: {bytes(request).hex().upper()}")
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, request,
                       expected_positive_sid=SID_11 + 0x40, allow_nrc=True)
    if resp is None or resp.responseType == 0:
        logger.log("硬重置失败", is_error=True)
        test_passed = False
    else:
        logger.log("硬重置成功")
    ...
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, request,
                       expected_positive_sid=SID_22 + 0x40, allow_nrc=True)
    if resp is None or resp.responseType == 0:
        logger.log("硬重置后读取运行分区失败", is_error=True)
        test_passed = False
    else:
        logger.log("硬重置后读取运行分区成功")
