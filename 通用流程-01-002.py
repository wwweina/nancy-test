...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-通用流程-01-002_{timestamp}.txt")
...
def main():
    ...
    # ---------- 功能寻址下进行默认会话测试 ----------
    logger.log("\n[功能寻址]----------10 83 请求----------")
    request = [SID_10, SUB_FUNC_EXTENDED]
    ...
    test_passed = False
    if resp is None:
        logger.log("功能寻址未收到响应（符合当前预期） -> 视为成功")
        test_passed = True
    elif resp.responseType == 1:
        logger.log("收到肯定响应，ECU 成功进入扩展会话")
        test_passed = True
    else:
        test_passed = False
