...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-通用流程-01-018_{timestamp}.txt")
...
def main():
    ...
    logger.log("\n-----------物理寻址85 82 测试------------")
    request = [SID_85, SUB_DIAGNOSTIC]
    logger.log(f"请求: {bytes(request).hex().upper()} (目标 CAN ID: 0x{ECU_ADDR:03X})")

    # 功能寻址：允许无响应（超时返回 None），但不允许否定响应
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, request,
                       expected_positive_sid=SID_85 + 0x40, allow_nrc=True)


    test_passed = False
    if resp is None:
        logger.log("物理寻址未收到响应（符合当前预期） -> 视为成功")
        test_passed = True
