...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-通用流程01-009_{timestamp}.txt")
...
def main():
    ...
    logger.log("\n----------------进入扩展模式------------")
    req_10 = [SID_10,SUB_EXTENDED_SESSION]
    ...
    logger.log("\n-----------功能寻址28 83 03 测试===无负反馈相应------------")
    request = [SID_28, SUB_DIAGNOSTIC, 0x03]
    logger.log(f"请求: {bytes(request).hex().upper()} (目标 CAN ID: 0x{ECU_ADDR:03X})")

    # 功能寻址：允许无响应（超时返回 None），但不允许否定响应
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, request,
                       expected_positive_sid=SID_10 + 0x40, allow_nrc=False)

    logger.log("\n----------------功能寻址28 03 03测试------------")
    req_28 = [SID_28,SUB_EXTENDED_SESSION, 0x03]
    logger.log(f"请求: {bytes(req_28).hex().upper()}")
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, req_28,
                       expected_positive_sid=None, allow_nrc=True)


    test_passed = False
    if resp.responseType == 1:
        logger.log("收到肯定响应，通讯控制关闭")
        test_passed = True
    else:
        test_passed = False
