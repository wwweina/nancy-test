...
SUB_CODE_SESSION = 0x02
SUB_EXTAND_SESSION = 0X03
SUB_CHECK_SESSION = 0xF1


# 例程 ID（2 字节）
ROUTINE_ID_1 = [0x02, 0x03]
ROUTINE_ID_2 = [0x00, 0x97]
...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-通用流程-01-055_{timestamp}.txt")
...
def main():
    ...
    steps = [
        ...
        ("扩展会话", [SID_10, SUB_EXTAND_SESSION], SID_10 + 0x40, True),
        ("启动例程", [SID_31, SUB_DEFAULT_SESSION] + ROUTINE_ID_1, SID_31 + 0x40, True),
        ("关闭DTC", [SID_85, SUB_CODE_SESSION], SID_85 + 0x40, False),   # 非必要，失败继续
        ("禁用收发", [SID_28, SUB_EXTAND_SESSION, 0x03], SID_28 + 0x40, True),
        ("编程会话", [SID_10, SUB_CODE_SESSION], SID_10 + 0x40, True),
    ]
    ...
    logger.log("\n----------F197信息读取----------")
    request = [SID_22, SUB_CHECK_SESSION, 0x97]
