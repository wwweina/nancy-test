...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-通用流程01-004_{timestamp}.txt")
...
def main():
    ...
    steps = [
        ...
        ("诊断会话控制", [SID_10, SUB_DIAGNOSTIC], SID_10 + 0x40, True),
    ]
    for name, req, expected, mandatory in steps:
        ...
        if resp is None and mandatory:
            ...
            return
    test_passed = False
    if steps and steps.sid == (SID_10 + 0x40):
        # 肯定响应，提取 DID 和数据
        # 假设 resp.data 包含完整响应（从第0字节开始）
        if len(steps.data) >= 2:
            did = (steps.data[0] << 8) | resp.data[1]
            data = steps.data[2:]
