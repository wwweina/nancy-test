...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-通用流程01-003_{timestamp}.txt")
...
def main():
    ...
    # ---------- 物理寻址进入默认模式/编程模式 ----------
    logger.log("\n-----------进入默认模式------------")
    request = [SID_10, SUB_DEFAULT_SESSION]
    ...
    test_passed = False
    if resp is not None and resp.responseType == 0 and resp.sid == SID_10:
