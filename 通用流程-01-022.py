...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-通用流程-01-022_{timestamp}.txt")
...
def main():
    ...
    logger.log("\n----------通讯控制打开 1----------")
    request = [SID_28, SUB_DEFAULT_SESSION,0x01]
    logger.log(f"请求: {bytes(request).hex().upper()} ")

    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, request,
                       expected_positive_sid=SID_28 + 0x40, allow_nrc=True)

    logger.log("\n-----------通讯控制打开 2------------")
    request = [SID_28, SUB_DEFAULT_SESSION, 0x3]
    logger.log(f"请求: {bytes(request).hex().upper()} ")
