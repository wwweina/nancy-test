...
def main():
    ...
    logger.log("\n----------------禁止记录DTC测试------------")
    req_85 = [SID_85, SUB_DIAGNOSTIC]
    logger.log(f"请求: {bytes(req_85).hex().upper()}")
    resp = uds_request(uds_if, ECU_ADDR, TESTER_ADDR, req_85,
                       expected_positive_sid=SID_85 + 0x40, allow_nrc=False)


    test_passed = False
    if resp is None:
        logger.log("请求失败或超时，无法获得响应", is_error=True)
        test_passed = False
    elif resp.responseType == 1:
