...
# DID 和测试数据
DID_STATUS_OF_THE_SIGNATURE_PUBLIC_KEY_H = 0xF1
DID_STATUS_OF_THE_SIGNATURE_PUBLIC_KEY_L = 0x60     # F160 签名公钥状态
...
class TestLogger:
    def __init__(self):
        ...
        self.log_path = os.path.join(os.getcwd(), f"UDS_TestLog-版本信息01-011_{timestamp}.txt")
...
def main():
    ...
    # ---------- 测试 22 服务 ----------
    logger.log("\n[测试] 发送 22 F0 F1 刷写尝试计数器查询")
    req_22 = [SID_22, DID_STATUS_OF_THE_SIGNATURE_PUBLIC_KEY_H, DID_STATUS_OF_THE_SIGNATURE_PUBLIC_KEY_L]
    ...
    test_passed = False
    if resp and resp.responseType == 0 and resp.sid == (SID_22 + 0x40):
        # 肯定响应，提取 DID 和数据
        # 假设 resp.data 包含完整响应（从第0字节开始）
        if len(resp.data) >= 3 and resp.data[0] == (SID_22 + 0x40):
            did = (resp.data[1] << 8) | resp.data[2]
            data = resp.data[3:]
